...
class index(QMainWindow, Ui_MainWindow):
    ...
    def init_classfiylist(self):
        ...
        self.classfiy.setFixedHeight(len(self.cList)*50)
    ...
    def initBtn(self):
        ...
        self.aboutBtn.clicked.connect(partial(self.change))
        self.aboutBtn.setCursor(QCursor(Qt.PointingHandCursor))


    def change(self):
        # script_path = os.path.join(os.path.dirname(sys.argv[0]), 'convert.py')
        # subprocess.Popen([sys.executable, 'convert.py'])
        self.bkshow = FileConverter()
        self.bkshow.show()
